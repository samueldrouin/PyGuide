...
ERREUR_CORRUPTION_INFORMATION = "La base de données est corrompue et doit être réparée avant de pouvoir être utilisée " \
                                "à nouveau."
...
ERREUR_CONSTRAINT_INFORMATION = "Une contrainte n'est pas respectée."
...
ANCIENNE_VERSION_INFORMATION = "La base de données est une ancienne version et ne peut être ouverte avec cette " \
                               "version du programme. Vous devez créer une nouvelle base de données pour continuer " \
...
def sql_error_handler(err):
    ...
    if err.nativeErrorCode() != "":
        ...
        msgbox.exec()
        return True
    elif err.type() == QSqlError.StatementError:
